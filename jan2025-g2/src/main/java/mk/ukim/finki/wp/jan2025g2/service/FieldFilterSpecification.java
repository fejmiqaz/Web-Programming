...
public class FieldFilterSpecification {
    public static <T, V extends Comparable<V>> Specification<T> greaterThan(Class<T> clazz, String field, Double value) {
        if (value == null) {
            return null;
        }
    ...
    public static <T> Specification<T> filterEquals(Class<T> clazz, String field, ParkType value) {
        if (value == null) {
            return null;
        }
        return (root, query, criteriaBuilder) -> criteriaBuilder.equal(fieldToPath(field, root), value);
